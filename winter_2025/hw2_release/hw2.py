...
def gaussian_kernel(size, sigma):
    ...
    for i in range(size):
        for j in range(size):
            x = i - k
            y = j - k
            kernel[i, j] = normalization * np.exp(- (x**2 + y**2) / (2.0 * sigma**2))
    ### END YOUR CODE
...
def partial_x(img):
    ...
    ### YOUR CODE HERE
    D_x = np.array([[-0.5, 0, 0.5]], dtype=np.float64)
    out = conv(img, D_x)
    ### END YOUR CODE
    ...
    for i in range(H):
        for j in range(W):
            ...
            if angle == 0:    # left-right
                neighbor1 = (i, j - 1)
                neighbor2 = (i, j + 1)
            elif angle == 45:  # diagonal up-right / down-left
                neighbor1 = (i - 1, j - 1)
                neighbor2 = (i + 1, j + 1)
            elif angle == 90:  # up-down
                neighbor1 = (i - 1, j)
                neighbor2 = (i + 1, j)
            elif angle == 135: # diagonal up-left / down-right
                neighbor1 = (i + 1, j - 1)
                neighbor2 = (i - 1, j + 1)
            ...
            n1 = safe_get(*neighbor1)
            n2 = safe_get(*neighbor2)
...
def link_edges(strong_edges, weak_edges):
    ...
    ### YOUR CODE HERE
    strong_coords = np.stack(np.nonzero(strong_edges)).T
    frontier = [(y, x) for (y, x) in strong_coords]
    
    while frontier:
        y, x = frontier.pop(0)  # pop from front (BFS-style)
...
def harris_corners(img, window_size=3, k=0.04):
    ...
    response = S_x2 * S_y2 - S_xy**2 - k * (S_x2 + S_y2)**2
    ### END YOUR CODE
...
def simple_descriptor(patch):
    ...
    feature = []
    ### YOUR CODE HERE
    feature = patch.astype(np.float32)
    mean_val = feature.mean()
    std_val = feature.std()
    if std_val < 1e-10:
        std_val = 1.0
    feature = (feature - mean_val) / std_val
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    ### YOUR CODE HERE
    # For each descriptor in desc1
    for i in range(dists.shape[0]):
        sorted_indices = np.argsort(dists[i])
        closest_idx = sorted_indices[0]
        second_closest_idx = sorted_indices[1]
        closest_dist = dists[i, closest_idx]
        second_closest_dist = dists[i, second_closest_idx]

        if closest_dist < threshold * second_closest_dist:
